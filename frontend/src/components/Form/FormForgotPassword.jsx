...
/*
  Form to send an email with instructions to reset the password or to change the password
  @param sendEmail: Boolean. Boolean to know if the email is going to be sent
  @param functionUser: Function. Function to send the email or change the password
  @param alert: Object. Alert object
  @returns {Object} - Form to send an email with instructions or to change the password
*/
      ...
      <div className="bg-gray-100 p-5 md:p-16  w-full sm:w-2/3 xl:w-1/3 py-16 mt-10 mx-auto rounded-md shadow-md hover:shadow-xl">
        <Formik
          ...
          validate={({ email, password, newPassword }) => {
            let errors = {};
            // Validation of the email and the password
            if (!sendEmail) {
              if (!password) {
                errors.password =
